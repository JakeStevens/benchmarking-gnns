...
def eval_pipeline(MODEL_NAME, dataset, params, net_params, write_file_name):
    t0 = time.time()
    per_epoch_time = []
    ...
    try:
        with tqdm(range(params['epochs'])) as t:
            for epoch in t:
                ...
                t.set_postfix(time=time.time()-start, test_f1=epoch_test_f1) 
...
def main():    
    ...
    print(f'node feat size: {net_params["in_dim"]}')
    print(f'edge feat size: {net_params["in_dim_edge"]}')
    num_classes = len(np.unique(np.concatenate(dataset.train[:][1])))
    net_params['n_classes'] = num_classes
    
    write_file_name = out_dir + 'results/eval_only_' + MODEL_NAME + "_" + DATASET_NAME + "_GPU" + str(config['gpu']['id'])

    if not os.path.exists(out_dir + 'results'):
        os.makedirs(out_dir + 'results')

    net_params['total_param'] = view_model_param(MODEL_NAME, net_params)
    eval_pipeline(MODEL_NAME, dataset, params, net_params, write_file_name)
